...
test("visitor count has its own footer row", async () => {
  ...
  assert.match(page, /© 2026 NELSON HSIEH · v1\.2\.34/);
...
test("the service worker replaces cached management assets when a frontend release ships", async () => {
  ...
  assert.match(page, /\/app\.js\?v=1\.2\.34/);
  assert.match(eventPage, /\/e\/app\.js\?v=1\.2\.34/);
  assert.match(worker, /good-days-github-v36/);
